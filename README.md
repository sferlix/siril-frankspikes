...
- **✨ Magic Wand** — one button: auto white balance + auto levels from the image's own pixels (a light-pollution color cast in the background gets neutralized, without touching genuine nebula/star color; the black/white points get a gentle, non-destructive nudge — see below), plus it (re)applies the focal-length-calibrated spike defaults (Length/Thickness/Intensity/Minimum diameter). Safe to click at any time, e.g. after fiddling with sliders and wanting a sane baseline back, without a full Reload
